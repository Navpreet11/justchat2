...
streamlitstyle = """
			<style>
			body,[class*="css"]{background-image: url("https://thepythoncodingbook.com/wp-content/uploads/2021/09/orbiting_planets_in_solar_system_using_python.jpg");
...
conn = sqlite3.connect('justchatdata.db')
cursor = conn.cursor()
...
def login():
    ...
    if st.button("Double tap to Log In"):
...
def signup():
    ...
    if st.button("Sign Up"):
        if not new_username or not new_password:
            st.error("Please enter your new username and password !")
        else:
...
def create_account(username, password):
    ...
    try:
        ...
        st.error("🧐 Username already taken.Choose a different username !")
